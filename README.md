...
# 5. Model Evaluation: `evaluate_3d_metrics.py`

## What It Does

This script calculates final segmentation metrics (Dice Similarity Coefficient, Intersection/Union, Sensitivity, Precision, and Hausdorff Distance 95) by comparing the 3D-cleaned Regions of Interest (ROIs) generated by your model against your manual ground-truth masks. It loads the images, binarizes them into strict 0 and 1 arrays, executes fast overlap math for volume metrics, and processes spatial math for boundary distance. It finishes by printing a consolidated final averages report for the entire dataset.
Note that it only applies to validation datasets.

## How to Use It

**Crucial Pre-Requisite:** At the start of the model training run, train_model.py will print the names of the specific datasets it randomly selected for the validation test at the end of each epoch. The specific validation dataset folders must be copied into the inference input directory before running generation_roi.py. Once the output datasets have been generated by generate_rois.py, evaluate_3d_metrics.py can be run.

After generating your 3D-cleaned masks with the `generate_rois` script, run this evaluation script to objectively score the model's accuracy. The script is controlled entirely via the terminal using command-line arguments.

**Command Structure:**
```bash
python evaluate_3d_metrics.py --preds [PREDICTIONS_PATH] --truths [GROUND_TRUTH_PATH]

```

### Argument 1: `--preds` (Required)

This argument defines the directory path containing the predicted ROIs generated by your model.

* **Use Case:** Point this directly to the output folder where your inference script just saved the AI-generated masks (e.g., `data/inference/output_tibia`).

### Argument 2: `--truths` (Required)

This argument defines the directory path containing your original, human-drawn validation masks.

* **Use Case:** Point this to the folder containing your perfect baseline answer key (e.g., `data/macro_tibia/masks`). The script uses relative paths to automatically pair each generated prediction with its exact corresponding ground-truth mask. If a ground-truth mask is missing for a specific prediction, the script will skip it and print a warning.
