...
train_transform = A.Compose([
    ...
    A.PadIfNeeded(min_height=960, min_width=960, border_mode=cv2.BORDER_CONSTANT, value=0, mask_value=0),

    # Original spatial augmentations
...
val_transform = A.Compose([
    # Ensure validation scans receive the exact same proportional downscale and padding
    A.LongestMaxSize(max_size=960, interpolation=cv2.INTER_AREA),
    A.PadIfNeeded(min_height=960, min_width=960, border_mode=cv2.BORDER_CONSTANT, value=0, mask_value=0),
...
# =============================================================================
def train_tibia(run_mode, epochs, model_path, images_base, masks_base, csv_path):
...
# dynamically assigns the correct variables before running the train_tibia() function.
...
# python train_tibia.py --target tibia --mode new

# 2 - Adding new datasets to your existing tibia model:
# python train_tibia.py --target tibia --mode finetune

# 3 - Resuming a crashed tibia training run:
# python train_tibia.py --target tibia --mode resume

# What the models do:
# train_tibia(run_mode="new", epochs=50): 
# Generates a completely new random split, starts at Epoch 0, sets LR to 1e-4.

# train_tibia(run_mode="resume", epochs=50): 
...
# train_tibia(run_mode="finetune", epochs=100): 
